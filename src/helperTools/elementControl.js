...
export function getMouseCoordinate(event) {
  const coordBox = document.getElementById(cf.MOUSE_COORD);
  let combinedText =
    "(" + event.offsetX + "," + (cf.HEIGHT - event.offsetY) + ")";
  coordBox.innerHTML = combinedText;
}

const canvas = document.getElementById(cf.CANVAS_NAME);
